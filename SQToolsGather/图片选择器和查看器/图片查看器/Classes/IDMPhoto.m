...
+ (NSArray *)photosWithURLs:(NSArray *)urlsArray isWifi:(BOOL)isWifi{
    ...
    for (id url in urlsArray) {
        if ([url isKindOfClass:[NSURL class]]) {
            ...
            NSString * newStr = newUrl.absoluteString;
            if ([newStr containsString:@"gif@"]) {
                NSRange range = [newStr rangeOfString:@"gif@"];
                newStr = [newStr substringToIndex:range.location + 3];
            }
            newUrl = [NSURL URLWithString:newStr];
            IDMPhoto *photo = [IDMPhoto photoWithURL:newUrl];
            photo.isWifi = isWifi;
            [photos addObject:photo];
        }
            ...
            IDMPhoto *photo = [IDMPhoto photoWithURL:[NSURL URLWithString:newStr]];
            photo.isWifi = isWifi;
            [photos addObject:photo];
        }
    }
...
- (void)loadUnderlyingImageAndNotify {
    ...
    if (self.underlyingImage) {
        ...
        if (_photoPath) {
            ...
            [manager loadImageWithURL:[weakSelf judgeWebImageCache:(NSString *)weakSelf.photoURL] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                ...
                if (image) {
                    ...
                    [weakSelf performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                }
            }];
            ...
            /*
            YYWebImageManager * mananer = [YYWebImageManager sharedManager];
            [mananer requestImageWithURL:[self judgeWebImageCache:_photoURL] options:YYWebImageOptionSetImageWithFadeAnimation progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                CGFloat progress = ((CGFloat)receivedSize)/((CGFloat)expectedSize);
...
- (NSURL *)judgeWebImageCache:(NSString *)WebImageUrl {
    ...
    for (NSString *key in icoArr) {
        if ([URL rangeOfString:key].location != NSNotFound) {
            NSString *newUrl =[URL substringToIndex:URL.length - key.length];
            if (self.isWifi == YES) {
                
                return [NSURL URLWithString:newUrl];
                
            }else{
                return [self getSDImageCacheNewUrl:newUrl];
               // return [self getYYimageCacheNewUrl:newUrl];
...
-(NSURL *)getSDImageCacheNewUrl:(NSString *)newUrl{
