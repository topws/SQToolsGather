...
- (id)initWithPhotoURLs:(NSArray *)photoURLsArray {
    if ((self = [self init])) {
        NSArray *photosArray = [IDMPhoto photosWithURLs:photoURLsArray isWifi:nil];
        _photos = [[NSMutableArray alloc] initWithArray:photosArray];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    // 查看原图
    _seePhooButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [_seePhooButton setFrame:CGRectZero];
    [_seePhooButton setAlpha:1.0f];
    _seePhooButton.hidden = _seePhooButtonIsHidden;
    [_seePhooButton addTarget:self action:@selector(seePhoto:) forControlEvents:UIControlEventTouchUpInside];
    [_seePhooButton setTitle:@"查看原图" forState:UIControlStateNormal];
    [_seePhooButton.titleLabel setFont:[UIFont boldSystemFontOfSize:11.0f]];
    [_seePhooButton setBackgroundColor:[UIColor colorWithWhite:0.1 alpha:0.5]];
    _seePhooButton.layer.cornerRadius = 3.0f;
    _seePhooButton.layer.borderColor = [UIColor colorWithWhite:0.9 alpha:0.9].CGColor;
    _seePhooButton.layer.borderWidth = 1.0f;
    ...
    [_doneButton addTarget:self action:@selector(savePhoto:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)viewWillLayoutSubviews {
    ...
    _seePhooButton.frame = [self frameForSeePhotoButtonAtOrientation:currentOrientation];
    // Remember index
...
- (void)performLayout {
    ...
    [self.view addSubview:_seePhooButton];
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            page.clouseBlock = ^(){
                [weakSelf doneButtonPressed:nil];
            };
...
- (void)configurePage:(IDMZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    IDMPhoto *currentPhoto = [self photoAtIndex:index];
    if (!_seePhooButtonIsHidden&&_seePhooButton.alpha == 0) {
       // currentPhoto.isCheckOriginalImage = YES;
    }
    page.photo = currentPhoto;
    ...
    //判断是否加载过原图
    if ([self _judgeWebImageCache:[currentPhoto.photoURL absoluteString]]) {
        _seePhooButton.alpha = 0;
    }else{
        _seePhooButton.alpha = 1;
    }
    
}


- (BOOL)_judgeWebImageCache:(NSString *)WebImageUrl {
    
    NSArray *icoArr = @[@"@!ico_xl_2x",@"@!ico_m_2x",@"@!m.jpg",@"@!l.jpg"];
    ...
    for (NSString *key in icoArr) {
        if ([WebImageUrl rangeOfString:key].location != NSNotFound) {
            ...
            UIImage *memoryImage = [cache getImageForKey:newUrl withType:YYImageCacheTypeAll];
             */
            UIImage *memoryImage = [[SDImageCache sharedImageCache] imageFromMemoryCacheForKey:newUrl];
            UIImage *cacheImage  = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:newUrl];
            if (memoryImage||cacheImage) {
                return YES;
            }
        }
...
- (CGRect)frameForToolbarAtOrientation:(UIInterfaceOrientation)orientation {
    ...
    return CGRectMake(0, 10, self.view.bounds.size.width, height);
}
...
- (CGRect)frameForDoneButtonAtOrientation:(UIInterfaceOrientation)orientation {
    ...
    return CGRectMake(screenWidth - 75, screenBound.size.height - 56, 55, 26);
}
- (CGRect)frameForSeePhotoButtonAtOrientation:(UIInterfaceOrientation)orientation {
    
    
    
    // if ([self isLandscape:orientation]) screenWidth = screenBound.size.height;
    CGRect screenBound = self.view.bounds;
    return CGRectMake(25, screenBound.size.height - 56, 55, 26);
}
- (CGRect)frameForCaptionView:(IDMCaptionView *)captionView atIndex:(NSUInteger)index {
...
//    NSLog(@"%ld == %@",index,_alphaArr);
    for (id key in _alphaArr) {
...
#pragma mark - Buttons
#pragma mark - Buttons  //查看原图
-(void)seePhoto:(UIButton *)btn{
    
    
 
    if (_alphaArr.count == 0) {
        btn.alpha = 0;
        [_alphaArr addObject:[NSString stringWithFormat:@"%ld",_currentPageIndex]];
    }else{
        for (id key in _alphaArr) {
            if ([key integerValue] != _currentPageIndex) {
//                NSLog(@"%ld",_currentPageIndex);
    ...
    NSArray *icoArr = @[@"@!s_2x.jpg",@"@!m_2x.jpg",@"@!l_2x.jpg",@"@!xl_2x.jpg",@"@!s_2x",@"@!m_2x",@"@!l_2x",@"@!xl_2x",@"@!ico_s_2x.jpg",@"@!ico_m_2x.jpg",@"@!ico_l_2x.jpg",@"@!ico_xl_2x.jpg",@"@!ico_s_2x",@"@!ico_m_2x",@"@!ico_l_2x",@"@!ico_xl_2x",@"@!m.jpg"];
    
    NSString *URL = photo.photoURL.absoluteString;
    NSLog(@"URL----%@-----",URL);
    if (!URL) {
        return;
    }
    for (NSString *key in icoArr) {
        if ([URL rangeOfString:key].location != NSNotFound) {
            NSURL *newUrl =[NSURL URLWithString:[URL substringToIndex:URL.length - key.length]];
            NSLog(@"URL----%@-----",newUrl.absoluteString);
            
            IDMPhoto *photo_new = [IDMPhoto photoWithURL:newUrl];
            photo_new.isCheckOriginalImage = YES;
            [_photos replaceObjectAtIndex:_currentPageIndex withObject:photo_new];
            
          
            IDMZoomingScrollView *pages = [self pageDisplayedAtIndex:_currentPageIndex];
            [pages resetProgress];
            pages.photo = photo_new;
            
            __weak IDMZoomingScrollView* weakPage = pages;
            photo_new.progressUpdateBlock = ^(CGFloat progress){
                NSLog(@"----%f---",progress);
                [weakPage setProgress:progress forPhoto:photo_new];
            };
